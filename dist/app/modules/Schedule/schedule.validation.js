...
exports.createScheduleSchema = zod_1.z
    ...
    slot: zod_1.z.array(zod_1.z.object({
        startTime: startTimeSchema,
        endTime: endTimeSchema
    })),
    availableSeats: zod_1.z.number().positive("available seats must be a positive number").optional(),
    availability: zod_1.z.enum(["Immediate Seating", "Open Reservations", "Waitlist"], {
        errorMap: () => ({ message: "{VALUE} is not supported" }),
    }).optional(),
    bookingFee: zod_1.z.number().nonnegative().default(0),
    paymentRequired: zod_1.z.enum(["Yes", "No"], {
        errorMap: () => ({ message: "{VALUE} is not supported" }),
    }).default('No'),
})
